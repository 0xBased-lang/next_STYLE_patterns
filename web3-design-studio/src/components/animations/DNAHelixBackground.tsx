...
import { DNAHelixEngine } from "@/lib/animations/DNAHelixEngine";
import type { DNAHelixConfig } from "@/lib/types/animation";
...
export function DNAHelixBackground({ config, className = "" }: DNAHelixBackgroundProps) {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const engineRef = useRef<DNAHelixEngine | null>(null);
  ...
  useEffect(() => {
    ...
    const engine = new DNAHelixEngine(config);
